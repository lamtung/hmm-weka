...
//			int pertubeNo = 1;
	...
	// Randomly change 1 row of the traistion matrix, use tabulist so that we only change rows that were not so often changed before (use frequency)
	// The tabu list only exist within the small changes , after a big change (change nbStates) it will be reset 
	// tabuList contains the frequency of row that has been changed, e.g row 1 changed 5 time --> tabuList[1] == 5
